...
// this code executed every times when a user(  creator or joinee )joins a new room
  const joinedRoom = ({ roomId, peerId }: IRoomParam) => {
    ...
    // the moment new user joins, add peerId to the key of roomid 
       ...
       //
       socket.on("ready", () => {
          //From the frontend when someone joined  we will emit the ready event 
          //then from the server we will emit an event to all clients connection that a new peer connection has added 
